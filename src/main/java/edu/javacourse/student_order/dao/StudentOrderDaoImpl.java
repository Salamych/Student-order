...
import edu.javacourse.student_order.domain.Address;
import edu.javacourse.student_order.domain.Adult;
...
import edu.javacourse.student_order.domain.StudentOrderStatus;
import edu.javacourse.student_order.domain.University;
...
public class StudentOrderDaoImpl implements StudentOrderDao {
 private final static Logger logger = LogManager.getLogger(DictionaryDaoImpl.class);  
    ...
    private static final String INSERT_ORDER
            ...
            + "	student_order_date, h_sur_name, h_given_name, "
            + "	h_patronymic, h_date_of_birth, h_passport_seria, "
    ...
    private static final String INSERT_CHILD
            ...
            + "	c_cetificate_number, c_cetificate_date, "
    ...
    public Long saveStudentOrder(StudentOrder so) throws DaoException {
        ...
        try ( Connection con = getConnection();  PreparedStatement stmt
            ...
            try {
                ...
                // Husband and Wife
                setParamdForAdult(stmt, 3, so.getHusband());
                setParamdForAdult(stmt, 18, so.getWife());
    ...
    private void setParamdForAdult(final PreparedStatement stmt, int start, Adult adult) throws SQLException {
...
//        return getStudentOrdersTwoSelect();
    ...
    private List<StudentOrder> getStudentOrdersOneSelect() throws DaoException {
        ...
        try ( Connection con = getConnection();  PreparedStatement stmt
                = con.prepareStatement(SELECT_ORDERS_FULL)) {
    ...
    private List<StudentOrder> getStudentOrdersTwoSelect() throws DaoException {
        List<StudentOrder> result = new LinkedList<>();
        try ( Connection con = getConnection();  PreparedStatement stmt
                = con.prepareStatement(SELECT_ORDERS)) {
            stmt.setInt(1, StudentOrderStatus.START.ordinal());
            int limit = Integer.parseInt(Config.getProperty(Config.DB_LIMIT));
            stmt.setInt(2, limit);
            ResultSet rs = stmt.executeQuery();
    ...
    private void fillSudentOrder(ResultSet rs, StudentOrder so) throws SQLException {
    ...
    private Child fillChild(ResultSet rs) throws SQLException {
        ...
        child.setCertificateNumber(rs.getString("c_cetificate_number"));
        child.setIssueDate(rs.getDate("c_cetificate_date").toLocalDate());
